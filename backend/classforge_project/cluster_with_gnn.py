...
#         return torch.cat([x1, x2, x3], dim=1)  # combined embedding

# # Initialize model
# model = ClassForgeEnsembleGNN(
#     in_channels=data.num_node_features,
#     hidden_channels=32,
#     embedding_size=16,
#     num_relations=len(set(data.edge_type.tolist()))
...
#     optimizer.zero_grad()
#     embeddings = model(data.x, data.edge_index, data.edge_type)
#     loss = embeddings.norm(p=2).mean()  # dummy loss to stabilize training
#     loss.backward()
#     optimizer.step()
#     if epoch % 25 == 0:
#         print(f"Epoch {epoch} | Loss: {loss.item():.4f}")
...
# model.eval()
# with torch.no_grad():
#     final_embeddings = model(data.x, data.edge_index, data.edge_type).cpu().numpy()


# # KMeans clustering for classroom allocation
# num_clusters = 3  # You can adjust this!
# # kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(X)
# # kmeans = KMeans(n_clusters=3, init='k-means++', random_state=42, n_init=10)
# kmeans = KMeans(n_clusters=num_clusters, random_state=42)
# cluster_labels = torch.tensor(kmeans.fit_predict(final_embeddings), dtype=torch.long)

# # Update graph with predicted cluster labels
# data.y = cluster_labels
# torch.save(data, "data/student_graph_clustered.pt")

# print("✅ GNN trained and students clustered into classrooms.")
# print(f"🧠 Cluster labels saved in: data/student_graph_clustered.pt")
